...
public final class CompanionPlayerEvents {
   ...
   public static void onPlayerLoggedIn(PlayerLoggedInEvent event) {
      if (event.getEntity() instanceof ServerPlayer player) {
         CompanionWorldData.StoredCompanionState state = CompanionWorldData.get(player.m_284548_()).getOrCreate(player.m_20148_());
         if (!state.hasSeenWelcome()) {
            AigfNetwork.sendToPlayer(player, new ClientboundOpenWelcomePacket(state.companionName, state.companionUuid != null));
         }

         if (state.companionUuid != null && player.m_284548_().m_8791_(state.companionUuid) instanceof CompanionEntity companion) {
   ...
   public static void onPlayerLoggedOut(PlayerLoggedOutEvent event) {
      if (event.getEntity() instanceof ServerPlayer player) {
         CompanionTabListManager.clear(player.m_20148_());
      }
   }
}
