...
public final class CompanionModernScreen extends Screen {
   private static final CompanionModernScreen.InventorySlotView[] INVENTORY_SLOTS = new CompanionModernScreen.InventorySlotView[]{
      new CompanionModernScreen.InventorySlotView(EquipmentSlot.MAINHAND),
      new CompanionModernScreen.InventorySlotView(EquipmentSlot.OFFHAND),
      new CompanionModernScreen.InventorySlotView(EquipmentSlot.HEAD),
      new CompanionModernScreen.InventorySlotView(EquipmentSlot.CHEST),
      new CompanionModernScreen.InventorySlotView(EquipmentSlot.LEGS),
      new CompanionModernScreen.InventorySlotView(EquipmentSlot.FEET)
   };
   ...
   private CompanionModernScreen.ScreenTab activeTab = CompanionModernScreen.ScreenTab.CHAT;
   ...
   protected void m_7856_() {
      ...
      CompanionModernScreen.UiLayout layout = this.layout();
   ...
   private void buildSidebarWidgets(CompanionModernScreen.UiLayout layout) {
      int tabWidth = layout.sidebarWidth - 28;
      int tabX = layout.sidebarX + 14;
      int tabY = layout.sidebarY + 146;

      for (CompanionModernScreen.ScreenTab tab : CompanionModernScreen.ScreenTab.values()) {
         this.chromeWidgets
            .add(
               (AbstractWidget)this.m_142416_(
                  new AigfSoftButton(tabX, tabY, tabWidth, 32, AigfUi.text(tab.label()), button -> this.switchTab(tab), () -> this.activeTab == tab, -2986325)
               )
            );
         tabY += 38;
      }
      ...
      this.followButton = (Button)this.m_142416_(this.actionButton(tabX, actionY, actionWidth, t("Follow me", "За мной"), CompanionActionIntent.FOLLOW));
      this.stayButton = (Button)this.m_142416_(
         this.actionButton(tabX + actionWidth + 12, actionY, actionWidth, t("Stay", "Стоять"), CompanionActionIntent.STAY)
      );
      this.sitButton = (Button)this.m_142416_(this.actionButton(tabX, actionY + 40, actionWidth, t("Sit", "Сидеть"), CompanionActionIntent.SIT));
      this.setHomeButton = (Button)this.m_142416_(
         this.actionButton(tabX + actionWidth + 12, actionY + 40, actionWidth, t("Set home", "Точка дома"), CompanionActionIntent.SET_HOME)
      );
   ...
   private void buildChatWidgets(CompanionModernScreen.UiLayout layout) {
      this.chatInput = (EditBox)this.m_142416_(
         new AigfSoftEditBox(
            this.f_96547_, layout.bodyX + 28, layout.bodyBottom() - 54, layout.bodyWidth - 190, 34, AigfUi.text(t("Talk to her...", "Напиши ей что-нибудь..."))
         )
      );
      this.chatInput.m_94199_(256);
      this.sendButton = (Button)this.m_142416_(
         new AigfSoftButton(
            layout.bodyRight() - 138, layout.bodyBottom() - 54, 110, 34, AigfUi.text(t("Send", "Отправить")), button -> this.sendChat(), () -> false, -2986325
         )
      );
   ...
   private void buildInventoryWidgets(CompanionModernScreen.UiLayout layout) {
      this.nameBox = (EditBox)this.m_142416_(
         new AigfSoftEditBox(this.f_96547_, layout.bodyX + 28, layout.bodyY + 72, layout.bodyWidth - 240, 34, AigfUi.text(t("Companion name", "Имя спутницы")))
      );
      this.nameBox.m_94199_(24);
      this.nameBox.m_94144_(this.snapshot.companionName());
      this.saveNameButton = (Button)this.m_142416_(
         new AigfSoftButton(
            layout.bodyRight() - 184,
            layout.bodyY + 72,
            156,
            34,
      ...
      this.giftFromHandButton = (Button)this.m_142416_(
         new AigfSoftButton(
            ...
            layout.bodyWidth - 56,
            34,
            AigfUi.text(t("Gift an item from your hand", "Подарить предмет из руки")),
      ...
      for (int index = 0; index < INVENTORY_SLOTS.length; index++) {
         CompanionModernScreen.SlotRect rect = this.inventorySlotRect(layout, index);
         CompanionModernScreen.InventorySlotView slotView = INVENTORY_SLOTS[index];
         Button fromHandButton = (Button)this.m_142416_(
            new AigfSoftButton(
               rect.x + rect.width - 170,
   ...
   private void buildAppearanceWidgets(CompanionModernScreen.UiLayout layout) {
      this.importNameBox = (EditBox)this.m_142416_(
         new AigfSoftEditBox(this.f_96547_, layout.bodyX + 28, layout.bodyY + 72, 210, 34, AigfUi.text(t("Nickname to import", "Ник для импорта")))
      );
      this.importNameBox.m_94199_(16);
      this.importSkinButton = (Button)this.m_142416_(
         new AigfSoftButton(
            layout.bodyX + 248, layout.bodyY + 72, 136, 34, AigfUi.text(t("Import skin", "Импорт скина")), button -> this.importSkin(), () -> false, -2986325
         )
      );
      this.previousSkinButton = (Button)this.m_142416_(
         new AigfSoftButton(layout.bodyX + 28, layout.bodyBottom() - 54, 54, 34, AigfUi.text("←"), button -> this.shiftSkinSelection(-1), () -> false, -1596986)
      );
      this.nextSkinButton = (Button)this.m_142416_(
         new AigfSoftButton(layout.bodyX + 92, layout.bodyBottom() - 54, 54, 34, AigfUi.text("→"), button -> this.shiftSkinSelection(1), () -> false, -1596986)
      );
      this.applySkinButton = (Button)this.m_142416_(
         new AigfSoftButton(
            layout.bodyRight() - 276,
            layout.bodyBottom() - 54,
            116,
            34,
      ...
      this.reloadSkinsButton = (Button)this.m_142416_(
         new AigfSoftButton(
            layout.bodyRight() - 148,
            layout.bodyBottom() - 54,
            120,
            34,
            AigfUi.text(t("Reload list", "Обновить список")),
   ...
   private void buildSettingsWidgets(CompanionModernScreen.UiLayout layout) {
      this.apiKeyBox = (EditBox)this.m_142416_(
         new AigfSoftEditBox(this.f_96547_, layout.bodyX + 28, layout.bodyY + 72, layout.bodyWidth - 56, 34, AigfUi.text("API Key"))
      );
      this.apiKeyBox.m_94199_(256);
      this.previousModelButton = (Button)this.m_142416_(
         new AigfSoftButton(layout.bodyX + 28, layout.bodyY + 130, 46, 34, AigfUi.text("←"), button -> this.cycleModel(-1), () -> false, -1596986)
      );
      this.modelIdBox = (EditBox)this.m_142416_(
         new AigfSoftEditBox(this.f_96547_, layout.bodyX + 84, layout.bodyY + 130, layout.bodyWidth - 260, 34, AigfUi.text("Model"))
      );
      this.modelIdBox.m_94199_(128);
      this.nextModelButton = (Button)this.m_142416_(
         new AigfSoftButton(layout.bodyRight() - 176, layout.bodyY + 130, 46, 34, AigfUi.text("→"), button -> this.cycleModel(1), () -> false, -1596986)
      );
      this.timeoutBox = (EditBox)this.m_142416_(
         new AigfSoftEditBox(this.f_96547_, layout.bodyRight() - 120, layout.bodyY + 130, 92, 34, AigfUi.text(t("Timeout", "Таймаут")))
      );
      this.contextTurnsBox = (EditBox)this.m_142416_(
         new AigfSoftEditBox(this.f_96547_, layout.bodyRight() - 120, layout.bodyY + 188, 92, 34, AigfUi.text(t("Context", "Контекст")))
      );
   ...
   private void switchTab(CompanionModernScreen.ScreenTab tab) {
      this.activeTab = tab;
      this.updateTabVisibility();
   }

   private void updateTabVisibility() {
      this.setVisible(this.chatWidgets, this.activeTab == CompanionModernScreen.ScreenTab.CHAT);
      this.setVisible(this.inventoryWidgets, this.activeTab == CompanionModernScreen.ScreenTab.INVENTORY);
      this.setVisible(this.appearanceWidgets, this.activeTab == CompanionModernScreen.ScreenTab.APPEARANCE);
      this.setVisible(this.settingsWidgets, this.activeTab == CompanionModernScreen.ScreenTab.SETTINGS);
   }
   ...
   public void m_88315_(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
      float time = (float)(Util.m_137550_() % 100000L) / 1000.0F;
      CompanionModernScreen.UiLayout layout = this.layout();
   ...
   private void renderSidebar(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawPanel(
         guiGraphics, layout.sidebarX, layout.sidebarY, layout.sidebarWidth, layout.sidebarHeight, -953870555, -518845163, AigfUi.colorAlpha(-1596986, 0.18F)
      );
      guiGraphics.m_280024_(
         layout.sidebarX + 1,
         layout.sidebarY + 1,
         layout.sidebarX + layout.sidebarWidth - 1,
         layout.sidebarY + 118,
         AigfUi.colorAlpha(-1596986, 0.14F),
         1182484
      );
      int orbX = layout.sidebarX + 32;
      int orbY = layout.sidebarY + 32;
      guiGraphics.m_280509_(orbX, orbY, orbX + 52, orbY + 52, AigfUi.colorAlpha(-10866092, 0.86F));
      guiGraphics.m_280024_(orbX + 4, orbY + 4, orbX + 48, orbY + 48, AigfUi.colorAlpha(-1596986, 0.6F), AigfUi.colorAlpha(-2986325, 0.34F));
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text("AIGF"), orbX + 13, orbY + 20, -133381, false);
      this.drawScaledText(guiGraphics, this.snapshot.companionName(), layout.sidebarX + 96, layout.sidebarY + 28, 1.4F, -133381);
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("android companion", "android-спутница")), layout.sidebarX + 96, layout.sidebarY + 54, -2505259, false);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("mood-aware, memory-rich, alive", "эмоции, память, характер")),
         layout.sidebarX + 96,
         layout.sidebarY + 68,
         -2505259,
         false
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Quick actions", "Быстрые действия")), layout.sidebarX + 18, actionTitleY, -133381, false);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("soft controls for her behavior", "мягкое управление её поведением")),
         layout.sidebarX + 18,
         actionTitleY + 14,
         -2505259,
         false
      ...
      guiGraphics.m_280614_(
         ...
         layout.sidebarX + 18,
         layout.sidebarY + layout.sidebarHeight - 24,
         -2505259,
         false
      );
   }

   private void renderHero(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawPanel(
         guiGraphics, layout.heroX, layout.heroY, layout.heroWidth, layout.heroHeight, -1087366098, -535359462, AigfUi.colorAlpha(-1596986, 0.22F)
      );
      guiGraphics.m_280024_(
         layout.heroX + 1, layout.heroY + 1, layout.heroX + layout.heroWidth - 1, layout.heroY + 70, AigfUi.colorAlpha(-1596986, 0.18F), 1182484
      );
      this.drawScaledText(guiGraphics, this.snapshot.companionName(), layout.heroX + 28, layout.heroY + 24, 1.9F, -133381);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("soft presence, sharp memory, personal tone", "мягкое присутствие, память, личный характер")),
         layout.heroX + 28,
         layout.heroY + 58,
         -2505259,
         false
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Current pulse", "Текущий пульс")), rightCardX + 16, layout.heroY + 34, -133381, false);
      guiGraphics.m_280614_(
         this.f_96547_, AigfUi.text(t("Mood", "Настроение") + "  " + this.snapshot.mood() + "/100"), rightCardX + 16, layout.heroY + 54, -2505259, false
      );
      guiGraphics.m_280614_(
         this.f_96547_, AigfUi.text(t("Trust", "Доверие") + "  " + this.snapshot.trust() + "/100"), rightCardX + 16, layout.heroY + 68, -2505259, false
      );
      guiGraphics.m_280614_(
         this.f_96547_, AigfUi.text(t("Energy", "Энергия") + "  " + this.snapshot.energy() + "/100"), rightCardX + 16, layout.heroY + 82, -2505259, false
      );
   }

   private void renderBody(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawPanel(guiGraphics, layout.bodyX, layout.bodyY, layout.bodyWidth, layout.bodyHeight, -719646437, -284030189, AigfUi.colorAlpha(-1596986, 0.16F));
   ...
   private void renderStatus(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      if (this.statusLine != null && !this.statusLine.isBlank()) {
         ...
         AigfUi.drawPanel(guiGraphics, statusX, statusY, statusWidth, 28, -1087760592, -702935010, AigfUi.colorAlpha(-1596986, 0.16F));
         guiGraphics.m_280614_(
            this.f_96547_, AigfUi.text(this.f_96547_.m_92834_(this.statusLine, statusWidth - 20)), statusX + 10, statusY + 10, -133381, false
         );
      }
   }

   private void renderChatTab(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawSectionTitle(
         guiGraphics,
         this.f_96547_,
         t("Conversation", "Разговор"),
         t(
            "Minecraft chat still works with @aigf, but this fullscreen view gives you the full flow.",
      ...
      for (String line : this.chatLines) {
         wrappedLines.addAll(this.f_96547_.m_92923_(AigfUi.text(line), conversationWidth - 24));
      }
      ...
      for (int index = start; index < wrappedLines.size(); index++) {
         guiGraphics.m_280648_(this.f_96547_, wrappedLines.get(index), conversationX + 12, y, -133381);
         y += 12;
      }
   }

   private void renderCareTab(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawSectionTitle(
         guiGraphics,
         this.f_96547_,
         t("Heart State", "Состояние сердца"),
         t("Your tone, distance, gifts, and calm moments all leave a mark on her.", "На неё влияют тон, дистанция, подарки и спокойные моменты рядом."),
         layout.bodyX + 28,
         layout.bodyY + 24,
         layout.bodyWidth - 56
      );
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Emotional readout", "Эмоциональный срез")), layout.bodyX + 42, lowerY + 18, -133381, false);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("Emotion", "Эмоция") + ": " + humanizeEmotion(this.snapshot.emotion().name())),
         layout.bodyX + 42,
         lowerY + 42,
         -2505259,
         false
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Long memory", "Долгая память")), layout.bodyX + 54 + leftCardWidth, lowerY + 18, -133381, false);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("Resentment", "Обида") + ": " + this.snapshot.resentment() + "/100"),
         layout.bodyX + 54 + leftCardWidth,
         lowerY + 42,
         -2505259,
         false
      );
      ...
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(this.statusLine.isBlank() ? t("No fresh hint right now", "Сейчас без новой подсказки") : this.statusLine),
         layout.bodyX + 54 + leftCardWidth,
         lowerY + 98,
         -532496,
         false
   ...
   private void renderInventoryTab(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawSectionTitle(
         guiGraphics,
         this.f_96547_,
         t("Personal Inventory", "Личный инвентарь"),
         t(
            "Give her a proper name, hand over gifts, and manage what she carries for herself.",
      ...
      for (int index = 0; index < INVENTORY_SLOTS.length; index++) {
         ...
         guiGraphics.m_280614_(this.f_96547_, AigfUi.text(localizedSlotName(slotView.slot())), rect.x + 16, rect.y + 14, -133381, false);
         Iterator var8 = this.f_96547_.m_92923_(AigfUi.text(itemName), rect.width - 34).iterator();
         if (var8.hasNext()) {
            FormattedCharSequence line = (FormattedCharSequence)var8.next();
            guiGraphics.m_280648_(this.f_96547_, line, rect.x + 16, rect.y + 34, -2505259);
         }
      }
   }

   private void renderAppearanceTab(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawSectionTitle(
         guiGraphics,
         this.f_96547_,
         t("Looks & Skins", "Внешность и скины"),
         t(
            "A softer dressing room for swapping local skins, built-ins, and imported player looks.",
      ...
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("Active skin", "Активный скин") + ": " + this.snapshot.activeSkinId()),
         layout.bodyX + 48,
         layout.bodyY + 148,
         -133381,
         false
      );
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Selected", "Выбран") + ": " + selectedSkin), layout.bodyX + 48, layout.bodyY + 168, -2505259, false);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("Available skins", "Доступно скинов") + ": " + this.skinIds.size()),
         layout.bodyX + 48,
         layout.bodyY + 188,
         -2505259,
         false
      );
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Folder", "Папка") + ": config/aigf/skins"), layout.bodyX + 48, layout.bodyY + 208, -2505259, false);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("Tip: keep 64x64 PNG skins for the cleanest result.", "Совет: используй PNG 64x64 для самого чистого результата.")),
         layout.bodyX + 48,
         layout.bodyY + 238,
         -532496,
         false
   ...
   private void renderSettingsTab(GuiGraphics guiGraphics, CompanionModernScreen.UiLayout layout) {
      AigfUi.drawSectionTitle(
         guiGraphics,
         this.f_96547_,
         t("AI Settings", "AI-настройки"),
         t(
            "OpenAI connection, model presets, timeouts, and context depth all live here in one calm place.",
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("API key", "API ключ")), layout.bodyX + 28, layout.bodyY + 58, -133381, false);
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Model", "Модель")), layout.bodyX + 84, layout.bodyY + 116, -133381, false);
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Timeout", "Таймаут")), layout.bodyRight() - 120, layout.bodyY + 116, -133381, false);
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(t("Context turns", "Контекст ходов")), layout.bodyRight() - 120, layout.bodyY + 174, -133381, false);
      AigfUi.drawCard(guiGraphics, layout.bodyX + 28, layout.bodyY + 246, layout.bodyWidth - 56, 84, false);
      guiGraphics.m_280614_(
         this.f_96547_,
         AigfUi.text(t("Character prompt is locked in-code", "Характерный промпт зашит в коде")),
         layout.bodyX + 42,
         layout.bodyY + 264,
         -133381,
         false
      ...
      guiGraphics.m_280614_(
         ...
         AigfUi.text(
            t(
               "She keeps her own personality, tone, and boundaries no matter which model is selected.",
               "Она сохраняет свой характер, тон и границы вне зависимости от выбранной модели."
            )
         ),
         layout.bodyX + 42,
         layout.bodyY + 286,
   ...
   private void drawStatCard(GuiGraphics guiGraphics, int x, int y, int width, String label, int value, int accent) {
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(label), x + 16, y + 16, -133381, false);
      this.drawScaledText(guiGraphics, String.valueOf(value), x + 16, y + 34, 1.6F, -133381);
      AigfUi.drawProgressBar(guiGraphics, this.f_96547_, x + 16, y + 66, width - 32, t("Level", "Уровень"), value, accent);
   }

   private void drawChip(GuiGraphics guiGraphics, int x, int y, String text, int accent) {
      int chipWidth = Math.min(170, this.f_96547_.m_92852_(AigfUi.text(text)) + 22);
      AigfUi.drawPanel(
         guiGraphics,
         x,
         y,
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(text), x + 10, y + 8, -133381, false);
   }
   ...
   private void drawScaledText(GuiGraphics guiGraphics, String text, int x, int y, float scale, int color) {
      ...
      guiGraphics.m_280614_(this.f_96547_, AigfUi.text(text), 0, 0, color, false);
      guiGraphics.m_280168_().m_85849_();
   }

   private CompanionModernScreen.SlotRect inventorySlotRect(CompanionModernScreen.UiLayout layout, int index) {
      int columnWidth = (layout.bodyWidth - 72) / 2;
      int x = layout.bodyX + 28 + index % 2 * (columnWidth + 16);
      int y = layout.bodyY + 170 + index / 2 * 92;
      return new CompanionModernScreen.SlotRect(x, y, columnWidth, 78);
   }

   private CompanionModernScreen.UiLayout layout() {
      int padding = 22;
      int sidebarWidth = Mth.m_14045_(this.f_96543_ / 5, 220, 280);
      int sidebarX = padding;
      int sidebarY = padding;
      int sidebarHeight = this.f_96544_ - padding * 2;
      int heroX = sidebarX + sidebarWidth + 18;
      int heroY = padding;
      int heroWidth = this.f_96543_ - heroX - padding;
      int heroHeight = 126;
      int bodyX = heroX;
      int bodyY = heroY + heroHeight + 16;
      int bodyWidth = heroWidth;
      int bodyHeight = this.f_96544_ - bodyY - padding - 44;
      return new CompanionModernScreen.UiLayout(
         sidebarX, sidebarY, sidebarWidth, sidebarHeight, heroX, heroY, heroWidth, heroHeight, bodyX, bodyY, bodyWidth, bodyHeight
      );
   }
   ...
   private static String humanizeAnalysis(String summary) {
      return switch (summary) {
         ...
         case "hurt_by_words,praised", "praised,hurt_by_words" -> t(
            "The conversation feels warm and painful at once.", "Разговор одновременно тёплый и болезненный."
         );
         default -> "";
      };
   }
   ...
   public boolean m_7933_(int keyCode, int scanCode, int modifiers) {
      if (this.activeTab == CompanionModernScreen.ScreenTab.CHAT && keyCode == 257) {
         this.sendChat();
         return true;
      } else {
   ...
   private enum ScreenTab {
      ...
      public String label() {
         return CompanionModernScreen.t(this.english, this.russian);
      }
   }
   ...
   private record UiLayout(
      ...
      int heroRight() {
         return this.heroX + this.heroWidth;
      }

      int bodyRight() {
         return this.bodyX + this.bodyWidth;
      }

      int bodyBottom() {
         return this.bodyY + this.bodyHeight;
      }
   }
}
