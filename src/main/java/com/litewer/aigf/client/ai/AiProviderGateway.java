...
final class AiProviderGateway {
   ...
   static String requestJsonResponse(
      AiProvider provider, ClientSettingsManager settings, String systemPrompt, String userPrompt, JsonObject schema, String schemaName, int maxTokens
      ...
      return switch (provider) {
         ...
         case OPENAI -> throw new IOException("OpenAI gateway is handled directly");
      };
   }
   ...
   static List<String> fetchModels(ClientSettingsManager settings) throws IOException, InterruptedException {
      return switch (settings.getProvider()) {
         ...
         case OPENAI -> fetchOpenAiModels(settings);
      };
   }
   ...
   private static List<String> fetchOpenAiModels(ClientSettingsManager settings) throws IOException, InterruptedException {
      HttpRequest request = HttpRequest.newBuilder(URI.create("https://api.openai.com/v1/models"))
         ...
         .build();
      JsonObject responseJson = sendJson(request);
      return parseModelIds(responseJson.getAsJsonArray("data"), "id", null);
   }

   private static List<String> fetchGeminiModels(ClientSettingsManager settings) throws IOException, InterruptedException {
      String endpoint = "https://generativelanguage.googleapis.com/v1beta/models?key=" + URLEncoder.encode(settings.getActiveApiKey(), StandardCharsets.UTF_8);
