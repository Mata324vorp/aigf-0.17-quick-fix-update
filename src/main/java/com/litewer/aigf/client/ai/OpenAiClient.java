...
public final class OpenAiClient {
   ...
   // ======================== REQUEST CHAT (CON OLLAMA) MODIFICADO ========================
   ...
   private static CompanionAiResult requestChat(String userMessage, CompanionSnapshot snapshot, ClientSettingsManager settings)
      ...
      if (settings.getActiveApiKey().isBlank()) {
         ...
         try {
            ...
            String emotionName = extractEmotion(structuredJson);
            String actionName = extractActionIntent(structuredJson);
            CompanionEmotion emotion = CompanionEmotion.fromName(emotionName);
            CompanionActionIntent actionIntent = CompanionActionIntent.fromName(actionName);
         ...
         // Llamada normal a OpenAI con API key (también tolerante por si acaso)
   ...
   private static String buildSystemPrompt(CompanionSnapshot snapshot, int maxContextTurns) {
      ...
      builder.append("You MUST include in your JSON response: 'moodDelta' (integer from -15 to 15) and 'trustDelta' (integer from -12 to 12). Positive if player is kind, negative if rude or disrespectful. ");
   ...
   private static String extractEmotion(JsonObject json) {
      JsonElement elem = json.get("emotion");
      if (elem == null) return "NEUTRAL";
   ...
   private static String extractActionIntent(JsonObject json) {
      JsonElement elem = json.get("actionIntent");
      if (elem == null) return "NOOP";
      if (elem.isJsonPrimitive()) return elem.getAsString();
      if (elem.isJsonObject()) {
         JsonObject obj = elem.getAsJsonObject();
         if (obj.has("type") && obj.get("type").isJsonPrimitive())
